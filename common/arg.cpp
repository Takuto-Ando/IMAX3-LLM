...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"-cs", "--mul-mat-chunk"}, "N",
        "chunk size for matrix multiplication (default: 64, set via GGML_MUL_MAT_CHUNK env var)",
        [](common_params & params, int value) {
            params.mul_mat_chunk_size = value;
            if (params.mul_mat_chunk_size <= 0) {
                params.mul_mat_chunk_size = 64;
            }
        }
    ));
    add_opt(common_arg(
        {"-cpu", "--use-cpu"},
        "force CPU path (equivalent to !defined(EMAX7), set via GGML_USE_CPU env var)",
        [](common_params & params) {
            params.use_cpu = true;
        }
    ));
