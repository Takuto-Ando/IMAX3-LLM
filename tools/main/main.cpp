...
#define NO_EMAX8LIB_BODY
#include "../../../../src/conv-c2e/emax8lib.c"
#endif

int final_token_number = 1000;
...
int main(int argc, char ** argv) {
    ...
    g_params = &params;
    monitor_time_start(0, T_MAIN); /* Nakashima */
    ...
    // Set GGML_MUL_MAT_CHUNK environment variable if specified via -cs/--mul-mat-chunk
    if (params.mul_mat_chunk_size > 0) {
        char env_str[64];
        snprintf(env_str, sizeof(env_str), "%d", params.mul_mat_chunk_size);
#if defined(_WIN32)
        if (_putenv_s("GGML_MUL_MAT_CHUNK", env_str) != 0) {
            LOG_ERR("%s: failed to set GGML_MUL_MAT_CHUNK environment variable\n", __func__);
        } else {
            LOG_INF("%s: setting GGML_MUL_MAT_CHUNK=%d\n", __func__, params.mul_mat_chunk_size);
        }
#else
        if (setenv("GGML_MUL_MAT_CHUNK", env_str, 1) != 0) {
            LOG_ERR("%s: failed to set GGML_MUL_MAT_CHUNK environment variable\n", __func__);
        } else {
            LOG_INF("%s: setting GGML_MUL_MAT_CHUNK=%d\n", __func__, params.mul_mat_chunk_size);
        }
    ...
    // Set GGML_USE_CPU environment variable if specified via -cpu/--use-cpu
    if (params.use_cpu) {
#if defined(_WIN32)
        if (_putenv_s("GGML_USE_CPU", "1") != 0) {
            LOG_ERR("%s: failed to set GGML_USE_CPU environment variable\n", __func__);
        } else {
            LOG_INF("%s: setting GGML_USE_CPU=1 (forcing CPU path)\n", __func__);
        }
#else
        if (setenv("GGML_USE_CPU", "1", 1) != 0) {
            LOG_ERR("%s: failed to set GGML_USE_CPU environment variable\n", __func__);
        } else {
            LOG_INF("%s: setting GGML_USE_CPU=1 (forcing CPU path)\n", __func__);
        }
    ...
    final_token_number = params.n_predict;
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        // predict
        monitor_time_start(0, T_MAIN_WHILE); /* Nakashima */
        if (!embd.empty()) {
    ...
    monitor_time_end(0, T_MAIN_WHILE); /* Nakashima */
        }
    monitor_time_end(0, T_MAIN); /* Nakashima */
    ...
    // ログを完全にフラッシュしてから次の処理へ
    common_log_pause(common_log_main());
    fflush(stdout);
    fflush(stderr);
    
    show_time_sep(); /* Nakashima */
