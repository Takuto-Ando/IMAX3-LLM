...
// precomputed f32 table for i16 (256 KB) (ggml-impl.h)
float ggml_table_f32_i16[1 << 16] __attribute__((aligned(32)));

volatile int IMAX_READY = 0;
int IMAX_DMABUF_LAST = 0;
volatile struct imax_dmabuf_map imax_dmabuf_map[IMAX_DMABUF_BLKS];
volatile struct th_inference_args th_inference_args[MAX_NTHREAD];
...
//     return GGML_COMMIT;
// }
// ggml/src/ggml.c:477行目付近
const char * ggml_version(void) {
    // 固定のバージョン文字列を直接返す
    return "0.9.4 (FIXED)";
}

// ggml/src/ggml.c:481行目付近
const char * ggml_commit(void) {
    // 固定のコミット文字列を直接返す
    return "custom_build";
}
...
struct ggml_context * ggml_init(struct ggml_init_params params) {
    ...
    if (is_first_call) {
        ...
        imax_dmabuf_map_register("ggml.c-ggml_init F16toF32", ggml_table_f32_f16, (1<<16)*sizeof(int), 1); /* Nakashima */
        for (int i = 0; i < (1 << 16); ++i) { /* Nakashima */
            ggml_table_f32_i16[i] = (float)(short)i;
        }
        imax_dmabuf_map_register("ggml.c-ggml_init I16toF32", ggml_table_f32_i16, (1<<16)*sizeof(int), 1); /* Nakashima */
